...
import { generateFamilyUnitPaths, generateSpousePath, calculateCanvasBounds } from '../../utils/HierarchicalTreeLayout';
    ...
    const connectionPaths = useMemo(() => {
        ...
        parentChildConnections.forEach(conn => {
            ...
            if (parentSpouseConn) {
                ...
                const spouseId = parentSpouseConn.from === conn.from ? 
                    parentSpouseConn.to : parentSpouseConn.from;
            ...
            {connectionPaths.map(conn => {
                ...
                return (
                    <g key={conn.id}>
                        ...
                        {showMarriageSymbol && <MarriageSymbol fromPos={conn.fromPos} toPos={conn.toPos} />}
                    </g>
                );
            })}
        </svg>
...
const MarriageSymbol = ({ fromPos, toPos }) => {
    ...
    return (
        <g>
            ...
            <path
                d={`M ${midX - offset - radius} ${midY} A ${radius} ${radius} 0 1 1 ${midX - offset + radius} ${midY}`}
                fill="none"
                stroke="#6b7280"
                strokeWidth="2"
                opacity="0.5"

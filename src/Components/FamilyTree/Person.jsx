...
const getCardWidth = () => (window.innerWidth <= 600 ? 110 : CARD_WIDTH);
const getCardHeight = () => (window.innerWidth <= 600 ? 140 : CARD_HEIGHT);
const getFontSize = (base) => (window.innerWidth <= 600 ? base - 2 : base);

const Person = ({ person, isRoot, onClick, rootId, tree, language, isNew, isSelected }) => {
    const cardWidth = getCardWidth();
    const cardHeight = getCardHeight();
    const fontSizeName = getFontSize(13);
    const fontSizeDetails = getFontSize(12);
    const fontSizeRelationship = getFontSize(11);
    ...
    return (
        <div
            ...
            style={{
                left: `${person.x - cardWidth / 2}px`,
                top: `${person.y - cardHeight / 2}px`,
                position: 'absolute',
                minWidth: cardWidth,
                maxWidth: 250,
                width: 'fit-content',
                ...
                padding: window.innerWidth <= 600 ? '1px 1px 3px 1px' : '6px 6px 12px 6px',
                background: isRoot
        ...
        >
            ...
            <div className="profile-pic-container flex items-center justify-center relative">
                <div className={`profile-pic-circle ${window.innerWidth <= 600 ? 'w-10 h-10' : 'w-16 h-16'} rounded-full overflow-hidden bg-gray-100 border-4 ${isRoot ? 'border-yellow-400 shadow-yellow-200' : person.gender === 'male' ? 'border-blue-300' : person.gender === 'female' ? 'border-pink-200' : 'border-gray-300'} shadow-lg group-hover:shadow-2xl transition-all duration-200`}>
                    <img
                ...
                {isNew && (
                  <span className="absolute -top-2 -right-2 bg-gradient-to-br from-green-400 to-teal-400 text-white rounded-full w-5 h-5 flex items-center justify-center font-bold text-xs shadow-md">+</span>
                )}
                {isSelected && !isNew && (
                  <span className="absolute -top-2 -right-2 bg-gradient-to-br from-green-500 to-green-700 text-white rounded-full w-5 h-5 flex items-center justify-center font-bold text-xs shadow-md"> 714</span>
                )}
            </div>
            {/* All info inside the card */}
            <div className="mt-2 w-full flex flex-col items-center justify-center">
                <span className="bg-white/90 px-2 py-0.5 rounded-full shadow text-gray-900 font-semibold border border-green-100 backdrop-blur-md tracking-wide text-center mb-1" style={{lineHeight: '1.2', fontSize: fontSizeName, maxWidth: '120px', overflowWrap: 'break-word', wordBreak: 'break-word'}}>
                    {person.name}
                </span>
                <span className="details text-xs text-gray-600 text-center font-medium mb-1" style={{fontSize: fontSizeDetails}}>
                    {person.gender.charAt(0).toUpperCase() + person.gender.slice(1)}{ageText}
                </span>
                {relationshipText && (
                    <span className="relationship inline-block px-2 py-0.5 rounded-full bg-green-100 text-green-700 font-semibold text-center tracking-wide shadow-sm mt-1" style={{fontSize: fontSizeRelationship, maxWidth: '80px', overflowWrap: 'break-word', wordBreak: 'break-word'}}>
                        {relationshipText}
                    </span>
                )}
            </div>
